...
path2vfnlib = 'D:\Dropbox\Caltech\VFN\VFN-Simulations\VFNlib';
addpath(path2vfnlib);
...
sys_props.pupil_shape = 'circ'; %Pupil shape ('circ' or 'keck')
...
apod_props.whichdesign = 'KAPOD-01'; % KAPOD-01 was faulty. KAPOD-02 is the replacement.
...
wvls = [2.0e-6 2.2e-6 2.4e-6];% wavelengths [meters]
% Beam sampling 
N_iris = 1024; % Number of samples across the beam at the iris
N_lambdaFnum = 4; % Number of samples per lambda*F# at the camera 
N_pix = 64;  % Number of pixels in final image
dxi = 45e-6; % Sample spacing in final image, or pixel pitch [meters]

outfile_label = 'vortex_nominal_1';
...
[E_iris,coords,w_I] = getEinput(fiber_props,sys_props,wvls,N_iris,N_grid);
...
[E_fp ,I_fp ] = getEfp(E_eff ,E_iris,sys_props,wvls,dx,dxi,N_pix);
[E_fp0,I_fp0] = getEfp(E_iris,E_iris,sys_props,wvls,dx,dxi,N_pix);
